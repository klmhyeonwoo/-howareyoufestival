class Market2Controller < ApplicationController
  def playingIndex
    @market2s = Market2.all.where(["status ilike true", "%#{params[:status]}%"]).order("id DESC") #id를 내림차순으로 정렬
  end

  def endIndex
    @market2s = Market2.all.where(["status ilike false", "%#{params[:status]}%"]).order("id DESC") #id를 내림차순으로 정렬
  end
